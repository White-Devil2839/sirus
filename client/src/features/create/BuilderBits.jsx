          ...
          <li key={label} className="flex items-center gap-2">
            <span
              className={`grid h-6 w-6 place-items-center rounded-full text-xs font-bold ${
                done ? 'bg-emerald-500 text-white' : active ? 'bg-brand-600 text-white' : 'bg-ink/15 text-muted'
              }`}
            >
              {done ? <Check size={13} /> : i + 1}
            </span>
            <span className={`font-semibold ${active ? 'text-ink' : 'text-muted/80'}`}>{label}</span>
...
export function SummaryPanel({ meta }) {
  ...
  return (
    <div className="sticky top-24">
      ...
      <div className="mt-3 overflow-hidden rounded-2xl bg-card shadow-card ring-1 ring-line/10">
        ...
        <div className="grid grid-cols-2 gap-px bg-ink/10 text-sm">
...
function Field({ label, value }) {
  return (
    <div className="bg-card p-3">
      <p className="text-[10px] font-bold uppercase tracking-wide text-muted/80">{label}</p>
